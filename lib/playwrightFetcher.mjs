...
import { resolveBrowserName } from './playwrightBrowserConfig.mjs';
...
export function resolvePlaywrightBrowser(browserName = process.env.BROWSER) {
  const normalized = resolveBrowserName(browserName);
  ...
  return {
    name: normalized,
    browserType: browserTypes[normalized],
...
async function ensureBrowser() {
  ...
  browserPromise = (async () => {
    const browserConfig = resolvePlaywrightBrowser();
    LOG(`[playwright] launching ${browserConfig.name} headless`);
...
export async function playwrightFetch(url, init = {}) {
  ...
  const waitUntil = init.__waitUntil ?? 'domcontentloaded';
