...
export async function getContent(url, options = {}) {
  ...
  while (counter <= maxTries) {
    try {
      await sleep(sleepMs);
      LOG(url, '[Start]', `(tried count: ${counter}, url: ${url}${options?._delayMs != null ? `, delayMs: ${options._delayMs}` : ''})`);
      ...
      if (typeof response.__decoded === 'string') {
        // Playwright 路徑：browser 已根據 Content-Type / <meta charset> 解好碼，
      ...
      const delayMs = options?._delayMs;
      LOG(url, '[Done]', `(tried count: ${counter}, url: ${url}, buf: ${len}${delayMs != null ? `, delayMs: ${delayMs}` : ''})`);
      break;
    } catch (e) {
